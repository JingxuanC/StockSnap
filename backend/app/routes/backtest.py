...
def run_backtest():
    ...
    if quotas and quotas[0]['backtest_used'] >= quotas[0]['backtest_limit']:
    ...
    if kline is None or hasattr(kline, 'data') and kline.data.empty:
    ...
    # 扣额度
    if not quotas:
        execute_insert("INSERT INTO user_quota (user_id, period_start, backtest_limit, backtest_used) VALUES (%s,%s,%s,1)", (g.user_id, period, settings.FREE_MONTHLY_QUOTA))
    else:
        execute_update("UPDATE user_quota SET backtest_used = backtest_used + 1 WHERE user_id = %s AND period_start = %s", (g.user_id, period))
...
@login_required
def history():
    page = request.args.get('page', 1, type=int); size = request.args.get('page_size', 20, type=int)
